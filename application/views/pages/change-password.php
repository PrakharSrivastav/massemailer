...
<html lang="en">
    ...
    <body>
        ...
        <section id="container" class="">
            <?php
            if ($this->session->userdata('is_logged_in') && $this->session->userdata('user_role') === '1') {
                $this->load->view("template/view-header");
                $this->load->view("template/view-sidebar");
                $this->load->view("content/view-change-password");
            } else if ($this->session->userdata('is_logged_in') && $this->session->userdata('user_role') === '2') {
                $this->load->view("template/view-header");
                $this->load->view("template/view-group-admin-sidebar");
                $this->load->view("content/view-change-password");
            } else if ($this->session->userdata('is_logged_in') && $this->session->userdata('user_role') === '3') {
                $this->load->view("template/view-header");
